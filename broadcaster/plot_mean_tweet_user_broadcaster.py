...
for anime_id, row in merged_data.iterrows():
    ...
    if os.path.exists(file_path):
        ...
        # '開始'の列が存在するか確認
        if '開始' in data.columns:
            data_date = pd.to_datetime(data['開始'], errors='coerce', format='%Y-%m-%d')

            broadcast_data = data[data['タイプ'] == '放送'].drop_duplicates(subset='放送局', keep='first')
            distribution_data = data[data['タイプ'] == '配信'].drop_duplicates(subset='配信サイト', keep='first')
            unique_data = pd.concat([broadcast_data, distribution_data])

            data_date = pd.to_datetime(unique_data['開始'], errors='coerce', format='%Y-%m-%d')
            mask = (data_date >= start) & (data_date <= end)

            filtered_data = unique_data[mask]
...
plt.xlabel('放送局数 + 配信サイト数')
plt.ylabel('平均週間ツイートユーザ数')
plt.title(f'放送局数+配信サイト数と平均週間ツイートユーザ数の散布図\nピアソンの相関係数: {correlation_coefficient:.2f}')
plt.legend()
plt.savefig("plot_mean_tweet_user_broadcaster.png")
...
# CSVとして保存する
result_df.to_csv('plot_mean_tweet_user_broadcaster.csv', index=False)
