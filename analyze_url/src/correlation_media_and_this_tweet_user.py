...
def calculate_overall_correlation(url_and_user_set):
    ...
    for variable in variables:
        temp_df = combined_df[[variable, 'tweet_users_count']].dropna()

        # 相関とp値を計算
        corr, p_value = pearsonr(temp_df[variable], temp_df['tweet_users_count'])

        correlations[variable] = (corr, p_value)

        # 線形回帰分析
        X = sm.add_constant(temp_df[variable])  # 定数項（切片）を追加
        y = temp_df['tweet_users_count']
        ...
        plt.title(f"{variable}とある週の週間ツイートユーザ数\n相関係数: {corr:.2f}, p値: {p_value:.2e}, 傾き: {slope:.2f}", fontsize=16)
        plt.xlabel(f'ある週の{variable}', fontsize=16)
        plt.ylabel('ある週の週間ツイートユーザ数', fontsize=16)
        plt.grid(True)
        plt.savefig(f"results/correlation_{variable}_with_this_week_user.png")
        plt.close()
