...
def calculate_overall_correlation(url_and_user_set):
    ...
    for id, df in url_and_user_set.items():
        for variable in variables:
            # 前週との変数の増減
            df[f'{variable}_change'] = df[variable].diff()
        # ユーザ数の増減
        df['user_count_change'] = df['tweet_users_count'].diff()
        # 次週へのシフト
        df['next_week_user_count_change'] = df['user_count_change'].shift(-1)
    ...
    for variable in variables:
        temp_df = combined_df[[f'{variable}_change', 'next_week_user_count_change']].dropna()
        corr, p_value = pearsonr(temp_df[f'{variable}_change'], temp_df['next_week_user_count_change'])
        correlations[variable] = (corr, p_value)

        # プロット
        plt.figure(figsize=(8, 6))
        plt.scatter(temp_df[f'{variable}_change'], temp_df['next_week_user_count_change'], alpha=0.5)
        plt.title(f"{variable}の増減と次週のユーザ数の増減の相関\n相関係数: {corr:.2f}, p値: {p_value:.2e}")
        plt.xlabel(f'前週からの{variable}の増減')
        plt.ylabel('次週のユーザ数の増減')
        plt.grid(True)
        plt.savefig(f"results/diff_correlation_{variable}_with_next_week_user_change.png")
        plt.close()

    # 相関係数とp値をコンソールに出力
    for variable, (corr, p_value) in correlations.items():
        print(f"{variable} - 相関係数: {corr}, p値: {p_value}")
