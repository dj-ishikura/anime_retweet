...
def calculate_and_plot(count_url_data, df_class):
    ...
    for anime in count_url_data:
        ...
        pixiv_count = anime['urls'].get('www.pixiv.net', 0)
        ...
        cluster_data[weekly_tweet_user_clusters]['x'].append(mean_tweet_user_count)
        cluster_data[weekly_tweet_user_clusters]['y'].append(pixiv_count)

        color = cluster_colors[weekly_tweet_user_clusters]
        plt.scatter(mean_tweet_user_count, pixiv_count, color=color)
        # plt.text(pixiv_count, mean_tweet_user_count, title, fontsize=9, alpha=0.7)
    ...
    # 凡例を作成
    plt.ylabel('pixivのURLを含むツイートの数', fontsize=18)
    plt.xlabel('平均週間ツイートユーザ数', fontsize=18)
    legend_labels = ['上昇', '下降', '山型', '横ばい']
    legend_colors = [cluster_colors[i] for i in range(4)]
    plt.legend([plt.Line2D([0], [0], color=color, marker='o', linestyle='') for color in legend_colors], legend_labels, fontsize=18)
    
    # グラフを保存
    plt.savefig("./results/pixiv_vs_mean_tweet_users.png")
    plt.close()
