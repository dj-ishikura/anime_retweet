...
def calculate_and_plot(count_url_data, df_class):
    ...
    for anime in count_url_data:
        ...
        youtube_count = 0
        for url in ["youtu.be", "www.youtube.com", "youtube.com"]:
            youtube_count += anime['urls'].get(url, 0)
        ...
        cluster_data[weekly_tweet_user_clusters]['x'].append(youtube_count)
        cluster_data[weekly_tweet_user_clusters]['y'].append(mean_tweet_user_count)

        color = cluster_colors[weekly_tweet_user_clusters]
        plt.scatter(youtube_count, mean_tweet_user_count, color=color)
        # plt.text(youtube_count, mean_tweet_user_count, title, fontsize=9, alpha=0.7)
    ...
    # 凡例を作成
    plt.xlabel('youtubeのURLを含むツイートの数', fontsize=18)
    ...
    # グラフを保存
    plt.savefig("./results/youtube_vs_mean_tweet_users.png")
    plt.close()
