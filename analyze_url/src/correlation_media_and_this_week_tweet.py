...
def calculate_overall_correlation(url_and_user_set):
    ...
    for variable in variables:
        temp_df = combined_df[[variable, 'user_tweet_count']].dropna()

        # 相関とp値を計算
        corr, p_value = pearsonr(temp_df[variable], temp_df['user_tweet_count'])

        correlations[variable] = (corr, p_value)

        # 線形回帰分析
        X = sm.add_constant(temp_df[variable])  # 定数項（切片）を追加
        y = temp_df['user_tweet_count']
        ...
        plt.title(f"{variable}とある週のツイート数\n相関係数: {corr:.2f}, p値: {p_value:.2e}, 傾き: {slope:.2f}", fontsize=16)
        plt.xlabel(f'ある週の{variable}', fontsize=16)
        plt.ylabel('ある週のツイート数', fontsize=16)
        plt.grid(True)
        plt.savefig(f"results/correlation_this_week_tweeet_with_{variable}.png")
        plt.close()
