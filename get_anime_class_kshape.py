...
n_clusters = 9
dba_km = KShape(n_clusters=n_clusters,
                          random_state=0)
...
sorted_cluster_df.to_csv('anime_class_kshape.csv', index=False)

from tslearn.barycenters import dtw_barycenter_averaging

def plot_and_save_graphs(title_list, anime_weekly_tweet_series, pred, n_clusters):
    # クラスタごとにディレクトリを作成する
    directory = 'cluster_plots_kshape'
    ...
    for i in range(n_clusters):
        ...
        plt.savefig(f'cluster_plots_kshape/cluster_{i}.png')
        plt.close()  # メモリを節約するためにプロットを閉じる


def save_images_to_pdf(n_clusters):
    pdf_filename = 'cluster_plots_kshape.pdf'
    
    with PdfPages(pdf_filename) as pdf_pages:
        for i in range(n_clusters):
            cluster_path = os.path.join('cluster_plots_kshape', f'cluster_{i}.png')
