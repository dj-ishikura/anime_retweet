...
output_dir="model_tweet_v2"
mkdir -p $output_dir

for followers_file in weekly_tweet_users_follower/*.jsonl; do
  wait_for_jobs
  id=$(basename "$followers_file" .jsonl) # ファイル名からアニメIDを取得
  output_file="${output_dir}/${id}.png"
  output_csv="${output_dir}/${id}.csv"
  followers_file="weekly_tweet_users_follower/${id}.jsonl"
  users_file="weekly_tweet_users_list/${id}.jsonl"

  mem=$(get_memory_allocation $followers_file)

  # Pythonプログラムを呼び出して処理を行う
  if [ ! -f "$output_file" ]; then
    echo $output_file
    J=$id M=$mem qcmd python model_tweet_v2.py $followers_file $users_file $output_file $output_csv $id
    sleep 1
  fi
done
