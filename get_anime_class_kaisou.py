...
# クラスタリングの実行
pred = agg_clustering.fit_predict(anime_weekly_tweet_series.reshape(len(anime_weekly_tweet_series), -1))
model = AgglomerativeClustering(distance_threshold=0, n_clusters=None, linkage='ward')
model = model.fit(anime_weekly_tweet_series.reshape(len(anime_weekly_tweet_series), -1))
...
sorted_cluster_df.to_csv('anime_class_kaisou.csv', index=False)

from tslearn.barycenters import dtw_barycenter_averaging

def plot_and_save_graphs(title_list, anime_weekly_tweet_series, pred, n_clusters):
    # クラスタごとにディレクトリを作成する
    directory = 'cluster_plots_kaisou'
    ...
    plt.savefig(f'cluster_plots_kaisou/cluster_kaisou.png')
    plt.close()
    ...
    for i in range(n_clusters):
        ...
        plt.savefig(f'cluster_plots_kaisou/cluster_{i}.png')
        ...
        plt.legend()
        plt.title(f'Cluster {i}')
        plt.savefig(f'cluster_plots_kaisou/cluster_{i}_.png')
        plt.close()  # メモリを節約するためにプロットを閉じる


def save_images_to_pdf(n_clusters):
    pdf_filename = 'cluster_plots_kaisou.pdf'
    
    with PdfPages(pdf_filename) as pdf_pages:
        cluster_path = sorted(glob.glob('cluster_plots_kaisou/*.png'))
        print(cluster_path)
