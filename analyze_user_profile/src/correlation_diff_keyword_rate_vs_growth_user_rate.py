    ...
    for filename in os.listdir(input_dir):
        if filename.endswith('.csv'):
            ...
            results.append({
                'title': title,
                'w_label': w_label,
                'keyword_diff': (df['keyword_ratio'].iloc[-1] - df['keyword_ratio'].iloc[0]) * 100,
...
def plot_correlation_vs_growth(df):
    ...
    for w_label in colors.keys():
        ...
        scatter = plt.scatter(df_subset['user_growth_rate'], df_subset['keyword_diff'], 
                              c=colors[w_label], label=w_label,
                              s=100, alpha=0.7)
    
    plt.xlabel('ユーザ数の増加率 (%)', fontsize=18)
    plt.ylabel('「アニメ」を含むユーザの割合の差 (%)', fontsize=18)
    ...
    for i, row in df.iterrows():
        plt.annotate(row['title'][:4], (row['user_growth_rate'], row['keyword_diff']), fontsize=16)
    
    plt.legend(title='週間クラスター', fontsize=16)
    
    plt.tight_layout()
    plt.savefig('plot/correlation_diff_keyword_rate_vs_growth_user_rate.png', dpi=300, bbox_inches='tight')
    plt.close()
