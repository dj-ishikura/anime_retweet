...
def plot_correlation_vs_growth(df):
    ...
    for w_label in colors.keys():
        ...
        scatter = plt.scatter(df_subset['user_growth_rate'], df_subset['correlation'], 
    ...
    for i, row in df.iterrows():
        plt.annotate(row['title'][:4], (row['user_growth_rate'], row['correlation']), fontsize=16)
    
    plt.legend(title='週間クラスター', fontsize=16)
    
    plt.tight_layout()
    plt.savefig('plot/correlation_weekly_user_vs_growth_user_rate.png', dpi=300, bbox_inches='tight')
    plt.close()
