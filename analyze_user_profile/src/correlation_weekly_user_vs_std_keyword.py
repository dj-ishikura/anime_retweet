...
def analyze_weekly_correlation(input_dir, anime_info):
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith('.csv'):
            ...
            # キーワード使用率の標準偏差を計算
            keyword_count_std = np.std(df['keyword_count'])
            ...
            results.append({
                ...
                'p_value': p_value,
                'w_label': w_label,
                'keyword_count_std': keyword_count_std,
                'user_growth_rate': (df['user_count'].iloc[-1] - df['user_count'].iloc[0]) / df['user_count'].iloc[0]
            })
    
    return pd.DataFrame(results)

def plot_correlation_vs_std(df):
    ...
    for w_label in colors.keys():
        ...
        scatter = plt.scatter(df_subset['keyword_count_std'], df_subset['correlation'], 
                              c=colors[w_label], label=w_label,
                              s=100, alpha=0.7)
    
    plt.xlabel('キーワードを含むユーザ数の標準偏差', fontsize=18)
    plt.ylabel('ユーザ数と「アニメ」キーワード使用率の相関', fontsize=18)
    plt.title('キーワード使用率の標準偏差と相関の関係', fontsize=18)
    ...
    for i, row in df.iterrows():
        plt.annotate(row['title'][:4], (row['keyword_count_std'], row['correlation']), fontsize=16)
    
    plt.legend(title='週間クラスター', fontsize=16)
    
    plt.tight_layout()
    plt.savefig('plot/correlation_vs_std_colored.png', dpi=300, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    plot_correlation_vs_std(results_df)
