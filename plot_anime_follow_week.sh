...
output_dir="follow_user_week_distribution"  # 出力ディレクトリのパスを設定するのだ
...
for jsonl_file in weekly_tweet_users_list/*.jsonl; do
    ...
    M=$mem qcmd python3 plot_anime_follow_week.py $jsonl_file $png_file
    # python3 plot_anime_follow_week.py $jsonl_file $png_file
    sleep 1
    wait_for_jobs
done
