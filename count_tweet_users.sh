...
tail -n +2 "$tsv_file" | while read LINE; do
    ...
    for period in 1 2 3 4; do
        ...
        output_csv="${output_dir}/${No}_${period}_week_tweet_counts.csv"
        output_png="${output_dir}/${No}_${period}_week_tweet_counts.png"
        if [ ! -f "$output_csv" ] || [ ! -f "$output_png" ]; then
            J=$No M=128 qcmd python count_tweet_users.py $input_file $period $output_csv $output_png $No
            sleep 1
        fi
    done
done
