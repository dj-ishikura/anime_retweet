...
class Model(nn.Module):
    def __init__(
        ...
        self.classifiers = nn.ModuleList([nn.Linear(hidden_size, 4) for _ in range(8)])
    ...
    def forward(
        ...
        if labels is not None:
            # 最後の層（感情極性）以外の損失計算
            loss_values = [self.loss_fn(logits[i], labels[:, i].long()) for i in range(len(logits))]

            # 損失の合計を取り、層の数で割る
            loss: FloatTensor = sum(loss_values) / len(logits)
